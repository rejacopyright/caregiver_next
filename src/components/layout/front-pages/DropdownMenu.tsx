// React Imports
import { CSSProperties, Fragment, MouseEvent, ReactNode, useEffect, useState } from 'react'
// Next Imports
...
const DropdownMenu = (props: Props) => {
  ...
  let landingUrl = p(config.landingUrl)
  ...
  return (
    <Tag {...(isBelowLgScreen && { className: 'flex flex-col' })}>
      <Typography
        color='text.primary'
        component={Link}
        className={clsx('flex items-center gap-2 font-medium plb-3 pli-1.5 hover:text-primary', {
          'text-primary':
            pathname === `${landingUrl}/payment` ||
            pathname === `${landingUrl}/pricing` ||
            pathname === `${landingUrl}/checkout` ||
            pathname === `${landingUrl}/help-center` ||
            pathname === `${landingUrl}/help-center/article/how-to-add-product-in-cart`,
        })}
        {...(isBelowLgScreen
          ? {
      ...
      <MenuWrapper
        ...
        <div className='flex flex-col gap-4'>
          ...
          {pageData.map((page, index) => (
            <Link
              key={index}
              href={landingUrl + page.href}
              className={clsx('flex items-center gap-3 focus:outline-none hover:text-primary', {
                'text-primary': pathname.includes(landingUrl + page.href),
              })}
        ...
        {!isBelowLgScreen && (
          <div className='flex bg-backgroundDefault p-2 rounded-md self-start'>
            <img
              src={dropdownImage}
              alt='dropdown image'
              className='rounded-md is-full bs-[350px]'
            />
          </div>
        )}
      </MenuWrapper>
