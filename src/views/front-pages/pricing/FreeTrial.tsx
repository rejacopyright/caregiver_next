// Next Imports
import Link from 'next/link'

import config from '@configs/config'
import Button from '@mui/material/Button'
import Grid from '@mui/material/Grid'
// MUI Imports
import Typography from '@mui/material/Typography'
import { p } from '@utils/fn'
// Third-party Imports
import clsx from 'clsx'

// Styles Imports
import frontCommonStyles from '@views/front-pages/styles.module.css'

const FreeTrial = () => {
  const landingUrl = p(config.landingUrl)

  return (
    <section className='bg-backgroundPaper'>
      <div className='bg-primaryLight'>
        <div
          className={clsx(
            'flex justify-between flex-wrap md:relative w-full',
            frontCommonStyles.layoutSpacing
          )}>
          <Grid container spacing={2}>
            <Grid size={{ xs: 12, md: 6 }}>
              <div className='flex flex-col gap-11 items-center md:items-start justify-center plb-10'>
                <div className='flex flex-col gap-2 max-md:text-center'>
                  <Typography variant='h4' color='primary.main'>
                    Still not convinced? Start with a 14-day FREE trial!
                  </Typography>
                  <Typography>
                    You will get full access to with all the features for 14 days.
                  </Typography>
                </div>
                <Button component={Link} href={`${landingUrl}/payment`} variant='contained'>
                  Start 14-Days Free Trial
                </Button>
              </div>
            </Grid>
            <Grid size={{ xs: 12, md: 6 }}>
              <div className='md:absolute md:inline-end-[90px] xl:inline-end-[2%] flex justify-center end-0 bottom-0'>
                <img
                  src='/images/front-pages/pricing-illustration.png'
                  alt='girl with laptop'
                  className='bs-[270px]'
                />
              </div>
            </Grid>
          </Grid>
        </div>
      </div>
    </section>
  )
}

export default FreeTrial
